...
// Quanto devono valere, in quota del budget, i giocatori che distinguono due rose perche'
// siano "strade diverse" e non la stessa strada con un riempitivo scambiato.
//
// Contare le teste era sbagliato, e sbagliato nel modo peggiore: scartava proprio la strada
// piu' utile. Sul listone vero, rinunciare all'attaccante da 124 e prendere quello da 127 e'
// uno scambio secco — una testa di differenza — e il filtro lo buttava via come "troppo
// simile", mentre teneva una rosa che cambiava tre riempitivi da tre crediti. Un giocatore
// da centoventiquattro e' un quarto del budget: e' la differenza piu' grande che una rosa
// possa avere, non la piu' piccola.
//
// E' la soglia sotto cui due rose sono la stessa cosa, non un criterio di scelta: sui casi
// veri non taglia niente, serve a non mostrare due volte la stessa strada.
const DIVERSITA_MINIMA = 0.05;
...
/** Quanto valgono i giocatori che distinguono due rose, in crediti previsti. */
function distanzaInCrediti(a, b) {
...
function battezza(piano, base, perno, presi) {
  ...
  // Il nome giusto e' l'acquisto che la strada rende possibile, se e' libero. Ma quando
  // quel nome se l'e' gia' preso un'altra strada non si scende lungo la lista fino al
  // riempitivo da sette crediti: si dice invece a cosa si rinuncia, che e' la scelta vera.
  // "Senza Svilar" e' un'informazione; "Con Delprato" era il nome di un dettaglio.
  const primo = nuovi[0] ? `Con ${nuovi[0].name}` : null;
  if (primo && !presi.has(primo)) return primo;
  const senza = perno ? `Senza ${perno.name}` : null;
  if (senza && !presi.has(senza)) return senza;
  for (const p of nuovi.slice(1)) {
    const nome = `Con ${p.name}`;
    if (!presi.has(nome)) return nome;
  }
  return senza || 'Alternativa';
}
...
export function generaScenari({
  ...
  // La scelta delle strade da mostrare.
  //
  // Prenderle semplicemente in ordine di punteggio non funziona: rinunciare al giocatore piu'
  // forte e' sempre la rinuncia che costa di piu', quindi quella strada finirebbe sempre in
  // fondo e non si vedrebbe mai. E' l'esatto contrario di quello che serve — "se non prendo
  // l'attaccante da centoventiquattro, cosa faccio?" e' la domanda piu' utile che questo tasto
  // possa rispondere.
  //
  // Quindi prima si copre un reparto per volta: la miglior strada che rinuncia a un attaccante,
  // quella che rinuncia a un centrocampista, quella che rinuncia al portiere. Sono tre forme
  // davvero diverse. I posti che restano si riempiono a punteggio.
  const soglia = (settings.budget || 500) * DIVERSITA_MINIMA;
  trovati.sort((a, b) => b.score - a.score);
  const scelti = [attuale];
  const aggiungi = (s) => {
    if (scelti.length >= quante || scelti.includes(s)) return false;
    if (!scelti.every((g) => distanzaInCrediti(s, g) >= soglia)) return false;
    scelti.push(s);
    return true;
  };
  ...
  for (const s of trovati) {
    ...
    if (aggiungi(s)) ruoliCoperti.add(ruolo);
  }
  for (const s of trovati) aggiungi(s);
  ...
  // Cosa cambia rispetto al piano, per poterlo mostrare.
  //
  // Elencare i giocatori piu' cari di ogni strada le faceva sembrare tutte uguali, ed era vero
  // il contrario di quello che si voleva dire: i piu' cari sono il nucleo che le strade hanno
  // in comune. Quello che distingue una strada e' proprio cio' che il nucleo non contiene.
  const idsBase = new Set(attuale.ids);
  const perPrezzo = (a, b) => b.prezzo - a.prezzo;
  const nomi = new Set();
    ...
    s.cambio = s.attuale
      ...
      : {
          entrati: s.ids
            ...
            .map((id) => ({ id, nome: nomePerId(players, id), prezzo: s.prezzi[id] ?? 0 }))
            .sort(perPrezzo),
          usciti: attuale.ids
            .filter((id) => !s.ids.includes(id))
            .map((id) => ({ id, nome: nomePerId(players, id), prezzo: attuale.prezzi[id] ?? 0 }))
            .sort(perPrezzo),
        };
  }
